#!/usr/bin/env python3
"""
Lookup one or more concepts from the psychology knowledge graph.
...
Examples:
  python lookup.py cognitive_reappraisal
  python lookup.py rumination worry
  python lookup.py --search "extinction fear"
  python lookup.py --fields cognitive_reappraisal definition_dense,mechanism,citations
...
SCRIPT_DIR = Path(__file__).parent.parent  # skills/psych-kg/
DB_CANDIDATES = [
    Path(os.environ.get("PSYCH_KG_PATH", "/nonexistent")),
    SCRIPT_DIR / "psychology_consensus_120_populated.json",  # bundled
    Path.cwd() / "psychology_consensus_120_populated.json",
    SCRIPT_DIR.parent.parent.parent / "psychology_consensus_120_populated.json",
    SCRIPT_DIR.parent / "psychology_consensus_120_populated.json",
]
...
def find_db():
    ...
    here = Path.cwd()
    for _ in range(6):
        candidate = here / "psychology_consensus_120_populated.json"
        if candidate.exists():
            return candidate
        here = here.parent
    raise FileNotFoundError(
        "Cannot find psychology_consensus_120_populated.json. "
        "Set PSYCH_KG_PATH env var or place the file in the working directory."
    )
...
def format_concept(c, fields=None):
    ...
    ALL_FIELDS = {
        ...
        "mechanism":                                ("Mechanism",               pf.get("mechanism", "")),
        "functional_role":                          ("Functional role",         pf.get("functional_role", "")),
        "common_measurement_or_operationalization": ("Measurement",             pf.get("common_measurement_or_operationalization", "")),
        "developmental_notes":                      ("Developmental notes",     pf.get("developmental_notes", "")),
        "clinical_or_applied_relevance":            ("Clinical/applied",        pf.get("clinical_or_applied_relevance", "")),
        "boundary_conditions":                      ("Boundary conditions",     pf.get("boundary_conditions", "")),
        "common_confusions":                        ("Common confusions",       pf.get("common_confusions", [])),
        "supersedes_or_reframes":                   ("Supersedes/reframes",     pf.get("supersedes_or_reframes", [])),
        "keep_outdated_terms_as_aliases_only":      ("Aliases (outdated)",      pf.get("keep_outdated_terms_as_aliases_only", [])),
        ...
        "consensus_notes":                          ("Consensus notes",         pf.get("consensus_notes", "")),
    ...
    if args[0] == "--search":
        ...
        if not matches:
            ...
            for score, c in matches[:10]:
                print(f"  [{score}] {c['slug']:<40} {c['label']}")
        return
